...
class MQ5():
    ...
    def __init__(self, Ro=10):
        ...
        # create an analog input channel on pin 3 for MQ5
        self.chan_MQ5 = AnalogIn(mcp, MCP.P3)
        
        self.LPGCurve = [2.3,-0.15,-0.38]   # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent"
                                            # to the original curve. 
                                            # data format:{ x, y, slope}; point1: (lg200, -0.15), point2: (lg10000,-0.80) 
        self.CH4Curve = [2.3,0.02,-0.39]    # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
                                            # data format:[ x, y, slope]; point1: (lg200, 0.02), point2: (lg10000, -0.68)
        self.H2Curve =[2.3,0.24,-0.25]      # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
                                            # data format:[ x, y, slope]; point1: (lg200, 0.24), point2: (lg10000, -0.17)
        self.AlcoholCurve =[2.3,0.54,-0.22] # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
                                            # data format:[ x, y, slope]; point1: (lg200, 0.54), point2: (lg10000,  0.18)
        self.COCurve =[2.3,0.59,-0.05]      # two points are taken from the curve. 
                                            # with these two points, a line is formed which is "approximately equivalent" 
                                            # to the original curve.
                                            # data format:[ x, y, slope]; point1: (lg200, 0.59), point2: (lg10000,  0.36)
                
        print("Calibrating MQ-5...")
        self.Ro = self.MQ5_Calibration()
    ...
    #          6.5, which differs slightly between different sensors.
    ############################################################################ 
    def MQ5_Calibration(self):
        val = 0.0
        for i in range(self.CALIBRATION_SAMPLE_TIMES):          # take multiple samples
            val += self.MQResistanceCalculation(self.chan_MQ5.value)
    ...
    def MQRead(self):
        ...
        for i in range(self.READ_SAMPLE_TIMES):
            raw_value += self.chan_MQ5.value
            rs += self.MQResistanceCalculation(self.chan_MQ5.value)
